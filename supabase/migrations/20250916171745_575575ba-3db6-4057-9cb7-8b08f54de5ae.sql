...
CREATE TABLE public.kirkpatrick_evaluations (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  enrollment_id UUID NOT NULL,
  level TEXT NOT NULL CHECK (level IN ('1', '2', '3', '4')),
  metric_name TEXT NOT NULL CHECK (metric_name IN ('satisfaction', 'knowledge_gain', 'application', 'business_impact')),
...
CREATE INDEX idx_kirkpatrick_evaluations_enrollment_id ON public.kirkpatrick_evaluations(enrollment_id);
CREATE INDEX idx_kirkpatrick_evaluations_level ON public.kirkpatrick_evaluations(level);
CREATE INDEX idx_kirkpatrick_evaluations_metric_name ON public.kirkpatrick_evaluations(metric_name);
CREATE INDEX idx_kirkpatrick_evaluations_evaluation_date ON public.kirkpatrick_evaluations(evaluation_date);
